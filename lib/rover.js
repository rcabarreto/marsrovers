...
Rover.prototype.move = function (pathPosition) {
  ...
  if (moved && fn.checkPosition(this.plateau.roverPosition, [this.x, this.y]) ) {
    ...
    // TODO: find alternative route to final position
    ...
    if (this.path[pathPosition+1] === 'M') {
      ...
      // need to jump over the rover in front of me!
      // check if there's space to go around by the left or right of the rover in front
      ...
      // TODO: make alternative route if next move is a turn
      ...
      // se for M depois L e depois M, faz isso: LMRML

      // se for M depois R e depois M, faz isso: RMLMR

      // se for M depois L ou R e acabou, fica onde está
    ...
    if (!global.visualMode)
      console.log('   - CANNOT MOVE! ROVER ON THE EDGE!');
